...
class ChatWindow:
    ...
    def _create_message_list(self):
        ...
        self.message_list_frame.pack(
            fill=tk.BOTH, expand=True, padx=20, pady=(20, 0))

        self.message_list = WrappedListBox(
            self.message_list_frame, width=80, height=20)
        ...
        self.scrollbar = tk.Scrollbar(
            self.message_list_frame, command=self.message_list.yview)
        self.scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        self.message_list.config(yscrollcommand=self.scrollbar.set)

        self.message_list.tag_configure("code", font=("Courier", 10))
    ...
    def _create_input_frame(self):
        ...
        self.input_box = tk.Text(
            self.input_frame, wrap="word", width=50, height=4)
    ...
    def handle_enter_key(self, event):
        self.send_message()
        # Prevents the default behavior (new line) and stops other bindings from being triggered
        return "break"
    ...
    def send_message(self):
        ...
        if message:
            ...
            response_thread = threading.Thread(
                target=self.generate_response, args=(message,))
    ...
    def generate_response(self, message):
        ...
        for partial_response in response_generator:
            self.root.after_idle(
                lambda pr=partial_response: self.update_message_list(pr, newline=False))
